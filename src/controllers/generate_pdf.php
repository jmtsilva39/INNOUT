...
function renderTable($report, $sumOfWorkedTime, $balance, $userName)
{
    ...
    $table = '<div class="container text-center align-middle">'; // Adicionando a classe container e text-center

    $table .= '<div style="position: fixed; top: 0; left: 0; width: 100%; margin: 0; padding: 0px;">';
    $table .= '<img src="https://i.ibb.co/cbFL9JH/logo.png" alt="Logo" style="width: 200px; height: auto; float: left;">' . '<h3 style="color: blue; text-align: center;">Superintendência de Água e Esgoto de Ourinhos</h3><hr>'; // Adicionando a imagem (altere o caminho conforme necessário)
    $table .= '</div>';
    
    $table .= '<h3 style="margin-bottom: 10px;margin-top: 65px">Relatório de Horas Trabalhadas no mês: '. $invertedPeriod .'<br>'. 'Funcionário: '  . $userName . '</h3>';
    $table .= '<table class="table align-middle d-flex justify-content-center"." style="width: auto; border-collapse: collapse; font-size: 14px;">'; // Adicionando a classe table para utilizar estilos padrão do Bootstrap
    $table .= '<thead>';
    $table .= '<tr style="background-color: #f0f0f0;">';
    $table .= '<th style="border: 1px solid #ddd; padding: 3px; min-width: 200px; text-align: center;">Dia</th>'; // Definindo uma largura mínima para a coluna
    ...
    $table .= '</tbody>';
    $table .= '</table>';
    $table .= '</div>'; // Fechando a div container
    $table .= '<footer style="position: fixed; bottom: 0; width: 100%; border-top: 1px solid #dee2e6; text-align: left; padding: 0px; margin-top: 70px;color: blue;">SUPERINTENDÊNCIA DE ÁGUA E ESGOTO DE OURINHOS<br>Av. Altino Arantes, 369, Centro – Fone (14) 3302-1000 – CEP 19900-031 – Ourinhos – SP - CNPJ 49.131.287/0001-88</footer>';
